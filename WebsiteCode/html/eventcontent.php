<!-- Php page to get data from upcomingevents table in database and display the results in the form of a table -->

<main class="event">
  <h1 class="anim">UPCOMING EVENTS</h1>
  <?php
// connect to database
...
//query for selecting row from table  limited to 5
  $user_check_query = "SELECT * FROM upcomingevents LIMIT 5";
...
// check if row isnt empty and then prints the row details 
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      echo "<tr><td>".$row["id"]."</td><td>".$row["event_name"]. "</td><td>" .$row["event_game"]."</td></td><td>" .$row["date"]."</td></tr>";
    }
    echo "</table>";
  } else {
  // if table is empty print message
    echo "No events planned currently";
  }
